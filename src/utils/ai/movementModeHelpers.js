 ...
 * 
 * Centralizes the decision logic for when combatants should use flight vs ground movement.
 * 
 * Key concepts:
 * - poorGroundRunner: true = combatant is clumsy on ground, prefers air
 * - preferFlight: true = combatant strongly prefers flying over ground running
 ...
 * 
 * Decision logic:
 * 1. Floaters (ghosts, elementals) Ã¢â€ â€™ always use flight, never ground
 * 2. Can't fly Ã¢â€ â€™ fraidered ground
 * 3. Already airborne Ã¢â€ â€™ stay airborne
 * 4. preferFlight: true Ã¢â€ â€™ take off for any meaningful distance (>5ft)
 * 5. poorGroundRunner: true Ã¢â€ â€™ fly for medium/long distances (>10ft)
 * 6. Good ground runners Ã¢â€ â€™ fly only for long reposition (>30ft)
 * 
 * @param {Object} fighter - Fighter object
...
export function shouldUseFlight(fighter, target, context = {}) {
  ...
  // Floaters (ghosts, air elementals) Ã¢â€ â€™ always "flying" / no ground run
  if (profile.movementMode === 'float' || profile.usesGroundRun === false) {
    return true;
  }

  // Can't fly at all Ã¢â€ â€™ fraidered ground
  ...
  // Species that strongly prefer flight (raptors, harpies, scouts, etc.)
  ...
  // Clumsy on ground + medium/long distance Ã¢â€ â€™ choose to fly
  if (profile.poorGroundRunner && distanceFeet > 10) {
    return true;
  }

  // Big grounded brutes (animals, gargoyles, baal_rog) can choose either.
