...
import { getSpellsForLevel, getAllSpellsFromDB } from "../data/combatSpells.js";
...
import { calculateVisibleCells, getVisibilityRange } from "../utils/visibilityCalculator.js";
...
import { canFly, isFlying, getAltitude, parseAbilities, hasNightvision, getNightVisionRange, calculateFlightMovement, applyBioRegeneration } from "../utils/abilitySystem.js";
import {
  canFighterFly,
  startFlying,
  landFighter,
...
function CombatPage({ characters = [], onUpdateCharacter = null }) {
  const navigate = useNavigate();
  const { activeParty, setActiveParty, refreshActiveParty } = useParty();
  ...
  const addLog = useCallback((message, type = "info", diceInfo = null) => {
    ...
    if (!logFlushTimerRef.current) {
      ...
      logFlushTimerRef.current = setInterval(() => {
        ...
        const batch = queue.splice(0, 5); // Flush up to 5 per tick to catch up during bursts
        if (batch.length > 0) {
          setLog((prev) => [...batch.reverse(), ...prev]);
        }
        if (queue.length === 0) {
          clearInterval(logFlushTimerRef.current);
          logFlushTimerRef.current = null;
  ...
  const spellAttemptBudgetRef = useRef(new Map()); // key: `${fighterId}:${playerAITurnToken}` -> attempts in this AI turn
  ...
  const executeSpellRef = useRef(null); // Store latest executeSpell to avoid deps churn
  const executePsionicPowerRef = useRef(null); // Store latest executePsionicPower to avoid deps churn
  ...
  const handleEngineEvent = useCallback((e) => {
    ...
    if (e.type === "ENGINE_CALL") {
      (async () => {
        ...
        if (!window?.engine?.call || !method) {
          addLog?.("⚠️ ENGINE_CALL missing window.engine.call or method", "warning");
          return;
        }
        ...
        setProjectiles((prev) => {
          ...
          return [
            ...prev,
            {
              ...
              durationMs: 450,
              currentX: projectile.from?.x,
              currentY: projectile.from?.y,
              currentZ: 0,          // ✅ add
              progress: 0,
            },
          ];
    ...
    if (e.type === "PROJECTILE_STEP") {
      const { projectileId, x, y, tNorm, arcZ, arcHeight } = e;
      // Update projectile position for smooth animation
      setProjectiles((prev) =>
        prev.map((p) => {
          ...
          return {
            ...
            currentX: x,
            currentY: y,
            currentZ: Number.isFinite(arcZ) ? arcZ : (p.currentZ ?? 0), // ✅ add
            arcHeight: Number.isFinite(arcHeight) ? arcHeight : p.arcHeight, // optional
            progress: tNorm,
          };
        })
      );
    ...
    if (e.type === "PROJECTILE_LAUNCHED") {
      ...
      if (projectileId && Number.isFinite(flightMs)) {
        setProjectiles((prev) =>
          prev.map((p) => (p.id === projectileId ? { ...p, durationMs: flightMs } : p))
        );
      }
      return;
    }
    ...
    if (e.type === "PPE_SPENT") {
      // Decrement caster PPE
      const { eid, remaining } = e;
      setFighters((prev) =>
        prev.map((f) =>
          f.id === eid
            ? { ...f, currentPPE: remaining }
            : f
        )
      );
    }
  ...
  }, [addLog, setFighters, setPositions, fighters, positions, turnCounter, setTemporaryHexSharing, activeCircles]);
  ...
  }, [ENGINE_ENABLED]); // Only recreate if ENGINE_ENABLED changes
  ...
  }, [clock]); // Only re-subscribe if clock changes (which is stable)
  ...
  const spellCastInProgressRef = useRef(new Set()); // Per-caster spell lock; always released in finally
  ...
  }, [meleeRound, isTargetImmuneToSpellDamageType, initializeThreatProfile]);
  ...
  const extractCharacterIdFromFighter = useCallback((fighter) => {
    const directId =
      fighter?.sourceCharacterId ||
      fighter?.characterId ||
      fighter?.originalCharacterId;
    if (directId) return String(directId);

    const fighterId = String(fighter?.id || fighter?._id || "");
    const match = fighterId.match(/^player-([a-f0-9]+)-/i);
    return match?.[1] || null;
  }, []);

  const collectAbilityStrings = useCallback((fighter) => {
    const rawEntries = [];
    const pushEntry = (entry) => {
      ...
      if (typeof entry === "object") {
        if (typeof entry.name === "string" && entry.name.trim()) {
          rawEntries.push(entry.name.trim());
        }
        if (typeof entry.description === "string" && entry.description.trim()) {
          rawEntries.push(entry.description.trim());
        }
      }
    };
    ...
    [
      ...
      fighter?.special_abilities,
      fighter?.racialAbilities,
    ].forEach((source) => {
  ...
  const getParsedAbilityProfile = useCallback((fighter) => {
    if (
      fighter?.abilities &&
      !Array.isArray(fighter.abilities) &&
      typeof fighter.abilities === "object"
    ) {
      return fighter.abilities;
    }
    return parseAbilities(collectAbilityStrings(fighter));
  }, [collectAbilityStrings]);

  const getFighterVisibilityProfile = useCallback((fighter) => {
    const parsedAbilities = getParsedAbilityProfile(fighter);
    const abilityText = collectAbilityStrings(fighter).join(" ").toLowerCase();
    const skillsFromAbilities = parsedAbilities?.skills || {};
    const prowlSkill = Math.max(
      Number(fighter?.prowlSkill) || 0,
      Number(fighter?.skills?.Prowl) || 0,
      Number(fighter?.skills?.prowl) || 0,
      Number(skillsFromAbilities.prowl) || 0,
      Number(getSkillPercentage(fighter, "Prowl")) || 0
    );
    const detectAmbushSkill = Math.max(
      Number(fighter?.detectAmbushSkill) || 0,
      Number(fighter?.skills?.["Detect Ambush"]) || 0,
      Number(skillsFromAbilities.detectambush) || 0,
      Number(getSkillPercentage(fighter, "Detect Ambush")) || 0
    );
    const hasInfravision = Boolean(
      fighter?.hasInfravision ||
        fighter?.hasNightvision ||
        hasNightvision(parsedAbilities) ||
        abilityText.includes("infravision") ||
        abilityText.includes("night vision") ||
        abilityText.includes("nightvision")
    );
    const nightvisionRange = Math.max(
      Number(fighter?.nightvisionRange) || 0,
      Number(parsedAbilities?.senses?.nightvision?.range) || 0,
      Number(getNightVisionRange(parsedAbilities)) || 0,
      hasInfravision ? 90 : 0
    );

    return {
      hasInfravision,
      hasNightvision: hasInfravision,
      nightvisionRange,
      prowlSkill,
      detectAmbushSkill,
      isProwling: Boolean(
        fighter?.isProwling || fighter?.hidden || fighter?.prowlState?.hidden
      ),
      hasSpecialSenses: hasSpecialSenses(fighter),
    };
  }, [
    collectAbilityStrings,
    getParsedAbilityProfile,
  ]);

  const isFighterConcealed = useCallback(
    (fighter) => Boolean(fighter?.hidden || fighter?.isProwling || fighter?.prowlState?.hidden),
    []
  );

  const stripFighterConcealment = useCallback((fighter, reason = "revealed") => {
    if (!fighter || !isFighterConcealed(fighter)) return fighter;
    return {
      ...fighter,
      hidden: false,
      isProwling: false,
  ...
  const revealFighterConcealment = useCallback((fighterOrId, reason = "revealed") => {
    const fighterId =
      typeof fighterOrId === "object"
        ? fighterOrId?.id || fighterOrId?._id
        : fighterOrId;
    if (!fighterId) return false;

    const liveFighter =
      (fightersRef.current || fighters).find((f) => f.id === fighterId) ||
      (typeof fighterOrId === "object" ? fighterOrId : null);
    if (!isFighterConcealed(liveFighter)) return false;

    setFighters((prev) =>
      prev.map((f) =>
        f.id === fighterId ? stripFighterConcealment(f, reason) : f
      )
    );
    addLog(
      `👁️ ${liveFighter?.name || "A fighter"} ${reason === "damage"
        ? "is revealed after taking damage"
        : reason === "movement"
          ? `reveals ${liveFighter?.type === "enemy" ? "its" : "their"} position by moving`
        : reason === "spell"
          ? `reveals ${liveFighter?.type === "enemy" ? "its" : "their"} position by casting`
          : reason === "psionic"
            ? `reveals ${liveFighter?.type === "enemy" ? "its" : "their"} position by using psionics`
            : "is revealed"}.`,
      "info"
    );
    return true;
  }, [addLog, fighters, isFighterConcealed, stripFighterConcealment]);

  const normalizeLootItemForPartyInventory = useCallback((item, itemType = null) => {
    const rawType = String(itemType || item?.type || item?.category || "misc").toLowerCase();
    let normalizedType = rawType;

    if (normalizedType === "inventory" || normalizedType === "item") {
      normalizedType = String(item?.category || item?.type || "misc").toLowerCase();
    }

    if (!["weapon", "armor", "consumable", "misc"].includes(normalizedType)) {
      if (item?.damage) normalizedType = "weapon";
      else if (item?.defense != null || item?.ar != null) normalizedType = "armor";
      else if (item?.effect) normalizedType = "consumable";
      else normalizedType = "misc";
    }
    ...
    return {
      ...
      type: normalizedType,
      damage: item?.damage || "",
      defense: Number(item?.defense ?? item?.ar) || 0,
      effect: item?.effect || "",
  ...
  const mergePartyInventoryItems = useCallback((existingItems = [], newItems = []) => {
    ...
    newItems.forEach((item) => {
      const normalized = normalizeLootItemForPartyInventory(item);
      const matchIndex = merged.findIndex((existing) =>
        existing.name === normalized.name &&
        existing.type === normalized.type &&
        (existing.damage || "") === normalized.damage &&
        Number(existing.defense || 0) === Number(normalized.defense || 0) &&
      ...
      if (matchIndex >= 0) {
        merged[matchIndex] = {
          ...merged[matchIndex],
          quantity:
            Math.max(1, Number(merged[matchIndex].quantity) || 1) +
            normalized.quantity,
        };
      } else {
    ...
    return merged;
  }, [normalizeLootItemForPartyInventory]);

  const persistLootToActiveParty = useCallback(async (itemsToAdd = []) => {
    if (!activeParty?._id) {
      addLog("⚠️ Load an active party before taking loot so it can be stored.", "warning");
      return false;
    }

    const normalizedItems = itemsToAdd.map((item) =>
      normalizeLootItemForPartyInventory(item, item.type)
    );
    const previousParty = activeParty;
    const nextInventory = mergePartyInventoryItems(
      activeParty.inventory || [],
      normalizedItems
    );

    setActiveParty((prev) =>
      prev?._id === activeParty._id ? { ...prev, inventory: nextInventory } : prev
    );

    try {
      const response = await axiosInstance.put(`/parties/${activeParty._id}`, {
        inventory: nextInventory,
      });
      const persistedParty = response.data?.party || response.data;
      if (persistedParty?._id) {
        setActiveParty(persistedParty);
      } else {
        await refreshActiveParty();
      }
      return true;
    } catch (error) {
      console.error("[CombatPage] Failed to persist loot to party inventory:", error);
      setActiveParty((prev) =>
        prev?._id === previousParty._id ? previousParty : prev
      );
      addLog("⚠️ Failed to save loot to party inventory.", "warning");
      return false;
    }
  }, [
    activeParty,
    addLog,
    mergePartyInventoryItems,
    normalizeLootItemForPartyInventory,
    refreshActiveParty,
    setActiveParty,
  ]);

  const persistXPRewards = useCallback(async (alivePlayerFighters = [], monster = null) => {
    if (!onUpdateCharacter || !monster) return;

    const aliveCharacterIds = new Set(
      alivePlayerFighters
        .map((fighter) => extractCharacterIdFromFighter(fighter))
        .filter(Boolean)
    );
    const eligibleCharacters = selectedParty.filter((character) =>
      aliveCharacterIds.has(String(character?._id || character?.id || ""))
    );
    ...
    try {
      updatedCharacters = grantXPFromEnemy(
        eligibleCharacters[0],
        monster,
        eligibleCharacters
      );
    } catch (error) {
      console.error("[CombatPage] Failed to calculate XP rewards:", error);
      return;
    }
    ...
    const persistedResults = await Promise.all(
      updatedCharacters.map(async (character) => {
        ...
        try {
          return await onUpdateCharacter(characterId, character);
        } catch (error) {
          console.error(
            `[CombatPage] Failed to persist XP for ${character?.name || characterId}:`,
            error
          );
          return character;
        }
    ...
    const persistedById = new Map(
      persistedResults.map((character) => [
        String(character?._id || character?.id || ""),
        character,
      ])
    );
    ...
    setSelectedParty((prev) =>
      prev.map((character) => {
        const id = String(character?._id || character?.id || "");
        return persistedById.get(id) || character;
      })
    );

    setFighters((prev) =>
      prev.map((fighter) => {
        const sourceId = extractCharacterIdFromFighter(fighter);
        const updatedCharacter = sourceId ? persistedById.get(String(sourceId)) : null;
        if (!updatedCharacter) return fighter;

        return {
          ...fighter,
          level: updatedCharacter.level ?? fighter.level,
          XP: updatedCharacter.XP ?? fighter.XP,
          nextLevelXP: updatedCharacter.nextLevelXP ?? fighter.nextLevelXP,
          xpHistory: updatedCharacter.xpHistory ?? fighter.xpHistory,
          totalXPEarned:
            updatedCharacter.totalXPEarned ?? fighter.totalXPEarned,
        };
      })
    );

    const leveledNames = persistedResults
      .filter((updatedCharacter, index) =>
        (updatedCharacter?.level || 1) >
        (eligibleCharacters[index]?.level || 1)
      )
    ...
    if (leveledNames.length > 0) {
      addLog(
        `⬆️ Level up! ${leveledNames.join(", ")} ${leveledNames.length === 1 ? "has" : "have"} advanced.`,
        "reward"
      );
    }
  }, [
    addLog,
    extractCharacterIdFromFighter,
    onUpdateCharacter,
    selectedParty,
  ]);
      ...
      const tryResolveAtHex = (hex, snapshot) => {
        ...
        for (const target of candidates) {
          ...
          const resolvedSnapshot = snapshot.isCriticalMiss
            ...
            : snapshot;
          if (wouldHitTarget(resolvedSnapshot, targetAR)) {
  ...
  const cancelPendingCombatWork = useCallback((reason = "combat-end") => {
    clearScheduledTurn();
    ...
    // Invalidate any already-scheduled async callbacks
    enemyTurnTokenRef.current += 1;
    playerAITurnTokenRef.current += 1;
    spellAttemptBudgetRef.current.clear();
    pendingTurnAdvanceRef.current = false;
    processingEnemyTurnRef.current = false;
    pendingEnemyTurnRef.current = false;
    processingPlayerAIRef.current = false;

    // Cancel timeline schedules (projectiles/impacts/turn-advance events)
    clockRef.current?.clearAll?.();
    addLog?.(`⛔ Cleared pending combat work (${reason})`, "info");
  }, [addLog, clearScheduledTurn]);

  const endCombatIfNoHostilesRemain = useCallback(
    (reason = "no-hostiles") => {
      if (combatEndCheckRef.current || !combatActiveRef.current) return false;
      const liveFighters = Array.isArray(fightersRef.current) ? fightersRef.current : [];
      const consciousPlayers = liveFighters.filter(
        (f) => f?.type === "player" && canFighterAct(f)
      );
      const consciousEnemies = liveFighters.filter(
        (f) => f?.type === "enemy" && canFighterAct(f)
      );

      if (consciousPlayers.length > 0 && consciousEnemies.length > 0) return false;

      combatEndCheckRef.current = true;
      combatOverRef.current = true;
      if (consciousEnemies.length === 0 && consciousPlayers.length > 0) {
        addLog("🎉 Victory! All enemies defeated!", "victory");
      } else if (consciousPlayers.length === 0 && consciousEnemies.length > 0) {
        addLog("💀 Defeat! All players have been defeated!", "defeat");
      } else {
        addLog("⚪ Combat ended: no opposing hostiles remain.", "info");
      }

      setCombatActive(false);
      cancelPendingCombatWork(reason);
      return true;
    },
    [addLog, canFighterAct, cancelPendingCombatWork]
  );
  ...
  }, [selectedSpell, selectedAction, currentFighter, fighters, selectedTarget, getTargetOptionsForAction]);
    ...
    if (fighters.length > 0 && turnIndex >= 0) {
      ...
      if (currentFighter && pendingMovements[currentFighter.id]) {
        ...
        if (occupant) {
          ...
          const movementType = String(pendingMove.movementType || "").toUpperCase();
          if (["RUN", "SPRINT", "CHARGE"].includes(movementType)) {
            revealFighterConcealment(currentFighter.id, "movement");
          }
        }
  ...
  }, [turnIndex, turnCounter, fighters, pendingMovements, addLog, isHexOccupied, revealFighterConcealment]);
  ...
  const eventToLogLine = useCallback((e) => {
    switch (e.type) {
      ...
      case "MISS":
        return `💨 ${nameOf(e.attackerId)} misses ${nameOf(e.targetId)}`;
      case "CRIT":
  ...
  const buildPositionsLite = useCallback((posMap) => {
    ...
    for (const k of Object.keys(src)) {
      ...
      // Standardize to {x,y}
      out[normalizedKey] = { x: Number(p.x ?? p.q ?? 0), y: Number(p.y ?? p.r ?? 0) };
    }
    return out;
  }, []);
  ...
  const scheduleEndTurn = useCallback(
    (delayOverride = null) => {
      ...
      // De-dupe: if a turn advance is already queued/in-flight, do not schedule again.
      if (pendingTurnAdvanceRef.current && (turnTimeoutRef.current != null || isActionBusy())) {
        return;
      }
      ...
      const tryEndTurn = () => {
        ...
        addLog(`🧪 tryEndTurn busy=${isActionBusy()} pending=${pendingTurnAdvanceRef.current}`, "info");
        if (isActionBusy()) {
  ...
  const confirmOverwatchHex = useCallback(
    (hex) => {
      ...
      if (isNoticeable) {
        ...
        for (const occId of occupantsNow) {
  ...
  const handleMoveSelect = useCallback((x, y) => {
    if (movementMode.active && selectedMovementFighter && positions[selectedMovementFighter]) {
      ...
      let combatant = fighters.find(f => f.id === selectedMovementFighter);
      ...
      if (combatant) {
        ...
        const breaksConcealment =
          movementMode.isRunning || selectedActionType === "charge";
        if (breaksConcealment && isFighterConcealed(combatant)) {
          const revealReason =
            selectedActionType === "charge" ? "charge" : "run";
          combatant = stripFighterConcealment(combatant, revealReason);
          setFighters((prev) =>
            prev.map((f) =>
              f.id === selectedMovementFighter ? { ...combatant } : f
            )
          );
          addLog(
            `👁️ ${combatant.name} gives away ${combatant.type === "enemy" ? "its" : "their"} position by ${revealReason === "charge" ? "charging" : "running"}!`,
            "info"
          );
        }
      }
  ...
  const validateWeaponRange = useCallback((
    ...
    // Spell range is authoritative here (prevents "in range" logs when actual cast path rejects range).
    if (actionType === "spell" || attackData?.type === "spell" || attackData?.spell || attackData?.damage === "by spell") {
      const spellLike = attackData?.spell || attackData;
      const attackerPos = attackerPosOverride || positionsRef.current?.[attacker?.id] || positions?.[attacker?.id];
      const defenderPos = defenderPosOverride || positionsRef.current?.[defender?.id] || positions?.[defender?.id];

      if (spellLike && attackerPos && defenderPos) {
        const spellRangeFt = getSpellRangeInFeet(spellLike);
        const distFt = calculateDistance(attackerPos, defenderPos);
        const inRange = spellRangeFt === Infinity || distFt <= spellRangeFt;
        return {
          canAttack: inRange,
          reason: inRange
            ? `Spell range ok (${Math.round(distFt)}ft ≤ ${spellRangeFt === Infinity ? "∞" : `${spellRangeFt}ft`})`
            : `Out of spell range (${Math.round(distFt)}ft > ${spellRangeFt}ft)`,
          shouldEndTurn: !inRange,
        };
      }

      return {
        canAttack: true,
        reason: "Spell selected (range fallback: missing positions)",
        shouldEndTurn: false,
      };
    }
    ...
    if (!weapon) {
      ...
      if (isRangedWeapon) {
        ...
        // Normalize ranges like "60 ft" to a numeric feet value.
        const numericRange =
          typeof weaponRange === "number"
            ? weaponRange
            : parseFloat(String(weaponRange).replace(/[^\d.]/g, "")) || 30;
        ...
        const canAttack = distanceForRangeCheck <= numericRange;
        const rangeType = isBreathWeapon ? 'breath weapon' : 'ranged';
        return {
          canAttack,
          reason: canAttack
            ? `Within ${rangeType} range (${Math.round(distanceForRangeCheck)}ft ≤ ${numericRange}ft)`
            : `Out of ${rangeType} range (${Math.round(distanceForRangeCheck)}ft > ${numericRange}ft)`,
          maxRange: numericRange,
          rangeInfo: isBreathWeapon ? `${rangeType} (${numericRange}ft cone/line)` : `ranged (${numericRange}ft)`,
          shouldEndTurn: !canAttack,
        };
      } else {
  ...
  const getFighterPPEEnvironment = useCallback((fighter) => {
    if (!fighter) return { onLeyLine: false, onNexus: false };
    const onLeyLine = Boolean(
      fighter?.onLeyLine ??
        fighter?.isOnLeyLine ??
        fighter?.magicContext?.onLeyLine ??
        fighter?.meta?.onLeyLine
    );
    const onNexus = Boolean(
      fighter?.onNexus ??
        fighter?.isOnNexus ??
        fighter?.magicContext?.onNexus ??
        fighter?.meta?.onNexus
    );
    return { onLeyLine, onNexus };
  }, []);

  const getAdjustedSpellPPECost = useCallback(
    (fighter, basePpeCost) => {
      const cost = Math.max(0, Number(basePpeCost) || 0);
      if (cost <= 0) return { baseCost: 0, adjustedCost: 0, onLeyLine: false, onNexus: false };
      const env = getFighterPPEEnvironment(fighter);
      const mult = env.onNexus ? 3 : env.onLeyLine ? 2 : 1;
      const adjustedCost = Math.max(0, Math.ceil(cost / mult));
      return {
        baseCost: cost,
        adjustedCost,
        onLeyLine: env.onLeyLine,
        onNexus: env.onNexus,
      };
    },
    [getFighterPPEEnvironment]
  );
  ...
  const handlePositionChange = useCallback((combatantId, newPosition, movementInfo = null) => {
    ...
    if (movementInfo && (movementInfo.action === 'RUN' || movementInfo.action === 'SPRINT')) {
      ...
      revealFighterConcealment(combatantId, "movement");
    } else {
      // Check if new position is off-board (fled/routed)
      const isOffBoard =
        newPosition.x < 0 ||
      ...
      if (combatant) {
        ...
        if (movementInfo?.movementType === "withdraw") {
          revealFighterConcealment(combatantId, "movement");
        }
      }
    }
  }, [fighters, positions, addLog, scheduleEndTurn, enqueueMoveAnimation, getMoveDurationMs, setRenderPositions, revealFighterConcealment]);
  ...
  const attack = useCallback(async (attacker, defenderId, bonusModifiers = {}) => {
    ...
    if (attackerIndex !== -1 && isFighterConcealed(updated[attackerIndex])) {
      updated[attackerIndex] = stripFighterConcealment(
        updated[attackerIndex],
        "attack"
      );
      stateAttacker = updated[attackerIndex];
      addLog(
        `👁️ ${stateAttacker.name} reveals ${stateAttacker.type === "enemy" ? "its" : "their"} position by attacking!`,
        "info"
      );
    }
    ...
    if (bonusModifiers?.attackDataOverride) {
      ...
      // AI-selected attacks must take precedence over UI selection state.
    ...
    if (attackData.type === "spell" || attackData.damage === "by spell" || attackData.spell) {
      ...
      if (!spellToCast?.name) {
        ...
        const isAI = attacker?.type === "enemy" || attacker?.aiControlled === true;
        if (isAI && attacker?.id) {
          setFighters(prev => prev.map(f => {
      ...
      const beforeRA =
        (fightersRef.current?.find((f) => f.id === attacker.id)?.remainingAttacks) ??
        (attackerInArray.remainingAttacks ?? 0);
      const spellResult = await executeSpellRef.current?.(attackerInArray, spellTarget, spellToCast);
      const spellSuccess = spellResult?.ok === true;
      const shouldConsumeAction = spellResult?.consumeAction === true;

      if (shouldConsumeAction && beforeRA > 0) {
        setFighters(prev => prev.map(f =>
          f.id === attackerInArray.id
            ? {
              ...f,
              remainingAttacks: Math.max(0, (f.remainingAttacks ?? beforeRA) - 1),
            }
            : f
        ));
        addLog(`🧪 spell attempt ${spellSuccess ? "succeeded" : "failed"}; consumed 1 action (was ${beforeRA})`, "info");
      } else {
        addLog(`🧪 spell attempt ${spellSuccess ? "succeeded" : "failed"}; no action consumed`, "info");
      }

      scheduleEndTurn(16); // delay so React can commit state before endTurn() reads fighters
      return;
    }
    ...
    if (
      ...
      // Shared helper: consume 1 action and end turn for AI when an invalid action is attempted.
      const burnActionAndEndIfAI = () => {
        const isAI = attacker?.type === "enemy" || attacker?.aiControlled === true;
        if (!isAI || !attacker?.id) return;
        setFighters(prev => prev.map(f => {
          if (f.id !== attacker.id) return f;
          const ra = Number(f.remainingAttacks ?? 0) || 0;
          return { ...f, remainingAttacks: Math.max(0, ra - 1) };
    ...
    try {
      ...
      if (isCriticalMiss && isProjectileAttack) {
        ...
        if (scatterHex) {
          addLog(
            `🟠 ${attacker.name} rolls NAT 1 — shot SCATTERS to (${scatterHex.x}, ${scatterHex.y})…`,
            "info"
          );
          ...
          for (const t of candidates) {
            let tAR = t.AR || t.ar || 10;
            if (combatTerrain && positions && positions[t.id]) {
              const coverBonus = getCoverBonus(
                { x: positions[t.id].x, y: positions[t.id].y },
                combatTerrain
              );
              if (coverBonus > 0) tAR += coverBonus;
            }

            const wouldHit = attackRoll >= tAR;
            if (wouldHit) {
      ...
      if (didHit) {
        ...
        if (finalDamage > 0 && isFighterConcealed(defender)) {
          const revealedDefender = stripFighterConcealment(defender, "damage");
          Object.assign(defender, revealedDefender);
          updated[defenderIndex] = defender;
          addLog(`👁️ ${defender.name} is revealed after taking damage!`, "info");
        }
        ...
        if (defenderAfterHit.type === "enemy") {
          ...
          if (remainingConsciousEnemies.length === 0 && !combatEndCheckRef.current) {
            ...
            addLog("🎉 Victory! All enemies defeated!", "victory");
            setCombatActive(false);
            cancelPendingCombatWork("victory-immediate");
        ...
        if (defenderAfterHit.currentHP <= 0) {
          ...
          if (defenderAfterHit.type === "enemy") {
            ...
            if (remainingConsciousEnemies.length === 0 && !combatEndCheckRef.current) {
              ...
              addLog("🎉 Victory! All enemies defeated!", "victory");
              setCombatActive(false);
              cancelPendingCombatWork("victory-attack-resolve");
          ...
          if (
            defenderAfterHit.type === "enemy" &&
            defenderAfterHit.currentHP <= -21 &&
            !defenderAfterHit.meta?.xpAwarded
          ) {
            defenderAfterHit = {
              ...defenderAfterHit,
              meta: {
                ...(defenderAfterHit.meta || {}),
                xpAwarded: true,
              },
            };
            updated[defenderIndex] = defenderAfterHit;
            ...
            if (alivePlayers.length > 0) {
              ...
              void persistXPRewards(alivePlayers, enemyData || defenderAfterHit);
          ...
          if (!combatEndCheckRef.current) {
            ...
            if (consciousPlayers.length === 0) {
              ...
              setCombatActive(false);
              cancelPendingCombatWork("defeat-immediate");
              ...
              addLog("🎉 Victory! All enemies defeated!", "victory");
              setCombatActive(false);
              cancelPendingCombatWork("victory-sync-check");
  ...
  const handleChargeAttack = useCallback((attacker, target) => {
    if (attacker?.id && isFighterConcealed(attacker)) {
      setFighters((prev) =>
        prev.map((f) =>
          f.id === attacker.id ? stripFighterConcealment(f, "charge") : f
        )
      );
      addLog(
        `👁️ ${attacker.name} gives away ${attacker.type === "enemy" ? "its" : "their"} position by charging!`,
        "info"
      );
    }
  ...
  const dispatchCombatAction = useCallback((type, payload) => {
    switch (type) {
      case "MOVE_SELECT":
      ...
      case "STRIKE_WITH_MOVEMENT":
        return handleStrikeWithMovement(
          payload.attacker,
          payload.movementHex,
          payload.target,
          payload.weapon
  ...
  const handlePlayerAITurn = useCallback((player) => {
    ...
    const context = {
      ...
      findBeePath,
      getTargetsInLine,
      onNoHostilesRemaining: endCombatIfNoHostilesRemain,
    };
  ...
  const executeAIAttack = useCallback(async ({ attacker, target, mode }) => {
    if (!ENGINE_ENABLED) {
      ...
      const flankingBonus = calculateFlankingBonus(positions[attacker.id], positions[target.id], positions, attacker.id);
      attack(finalAttacker, target.id, flankingBonus > 0 ? { flankingBonus } : {});
      return;
    }
    ...
    const hpById = {};
    for (const f of (fightersRef.current ?? fighters)) {
      const id = f.id ?? f._id;
  ...
  }, [ENGINE_ENABLED, attack, resolveEnemyEffectiveAttack, calculateFlankingBonus, positions, addLog, eventToLogLine, applyAttackDelta, ammoCount, fighters, onEndTurnFull]);
  ...
  const handleEnemyTurn = useCallback(async (enemy, source = "unknown", meta = {}) => {
    ...
    if (processingEnemyTurnRef.current) {
      const turnKeyForLog = metaTurnKey ?? `${enemy?.id}|round:${meleeRound}|turnIndex:${turnIndex}`;
    ...
    if (playerTargets.length === 0) {
      ...
      if (allPlayers.length > 0) {
        ...
        if (endCombatIfNoHostilesRemain("enemy-no-targets")) return;
          ...
          if (retreatDestination) {
            ...
            addLog(
              `[AI] ${enemy.name} withdraws ${Math.round(
                retreatDestination.distanceFeet
              )}ft to (${retreatDestination.position.x}, ${retreatDestination.position.y
              }).`,
              "ai"
            );
          } else if (!currentPos) {
    ...
    if (positions && positions[enemy.id] && positions[target.id]) {
      ...
      if (!rangeValidation.canAttack) {
        ...
        const isSpellRangePrecheck = /spell/i.test(String(rangeValidation.reason || ""));
        addLog(
          isSpellRangePrecheck
            ? `ℹ️ ${enemy.name} spell precheck: ${rangeValidation.reason} (authoritative check occurs at cast).`
    ...
    scheduleEndTurn(0);
      ...
      if (pendingEnemyTurnRef.current && !didCompleteTurn) {
        pendingEnemyTurnRef.current = false;
        const stillCurrent = fighters[turnIndex]?.id === enemy?.id;
        const coalescedTurnKey = `${enemy?.id}|round:${meleeRound}|turnIndex:${turnIndex}`;
        ...
        if (stillCurrent && !combatOverRef.current && combatActive && !alreadyClaimedAndActive) {
          const fighterToRun = fighters[turnIndex] || enemy;
          addLog?.(`🔄 Coalesced blocked call, re-scheduling ${fighterToRun?.name}`, "info");
          setTimeout(() => handleEnemyTurn(fighterToRun, "coalesced", { turnKey: coalescedTurnKey }), 0);
        }
      }
    }
  }, [
  ...
  useEffect(() => {
    ...
    // Stable turnKey: changes only when active fighter changes (not remainingAttacks, turnCounter, etc.)
    const currentTurnKey = `${currentFighter.id}|round:${meleeRound}|turnIndex:${turnIndex}`;
    ...
    if (currentFighter.type === "player") {
      if (aiControlEnabled) {
        ...
        if (!processingPlayerAIRef.current) {
          // Block if we already scheduled (or are currently processing) this same turn
          if (lastPlayerAIScheduleTurnKeyRef.current === currentTurnKey) {
            if (playerAITimerRef.current) return;
            if (processingPlayerAIRef.current) return;
          }
          lastPlayerAIScheduleTurnKeyRef.current = currentTurnKey;
          if (playerAITimerRef.current) {
            clearTimeout(playerAITimerRef.current);
            playerAITimerRef.current = null;
          }
      ...
      if (!processingEnemyTurnRef.current) {
        // Block if we already scheduled (or are currently processing) this same turn
        if (lastEnemyScheduleTurnKeyRef.current === currentTurnKey) {
          if (enemyTurnTimerRef.current) return;
          if (processingEnemyTurnRef.current) return;
        }
        lastEnemyScheduleTurnKeyRef.current = currentTurnKey;

        // Clear stale timer if it was for a different turn
        const timerAlreadySet = !!enemyTurnTimerRef.current;
        if (enemyTurnTimerRef.current) {
          clearTimeout(enemyTurnTimerRef.current);
          enemyTurnTimerRef.current = null;
        }
        ...
        const scheduledFighterId = currentFighter?.id;
        const scheduledTurnKey = currentTurnKey;
        const token = ++enemyTurnTokenRef.current;
        addLog?.(`🧪 scheduleEnemyTurnStart ${currentFighter?.name} in 0ms (reason=effect-turn-advance)`, "info");
        startTransition(() => {
          enemyTurnTimerRef.current = setTimeout(() => {
            enemyTurnTimerRef.current = null;
            if (token !== enemyTurnTokenRef.current) return; // B. Stale token, bail

            // Still-current-fighter guard: verify it's still this fighter's turn using latest refs
            const latestFighter = fightersRef.current?.[turnIndexRef.current];
            if (!latestFighter || latestFighter.id !== scheduledFighterId) return;

            const latestTurnKey = `${latestFighter?.id}|round:${meleeRoundRef.current}|turnIndex:${turnIndexRef.current}`;
            if (latestTurnKey !== scheduledTurnKey) return;
            ...
            handleEnemyTurn(latestFighter, "effect-turn-advance", { turnKey: scheduledTurnKey, token });
          }, 0);
        });
      }
    }
  ...
  function addCreature(creatureData, customNameOverride = null, levelOverride = null, armorOverride = null, weaponOverride = null, ammoOverride = null, fighterTypeOverride = null) {
    ...
    if (creatureData.playable) {
      // Ensure visual/footprint from bestiary (source of truth) so 3D model loads
      const bestiaryEntry = getAllBestiaryEntries(bestiary).find(
        (c) => c.id === creatureData.id || c.name === creatureData.name
      );
      const creatureWithVisual = bestiaryEntry?.visual || bestiaryEntry?.footprint
        ? { ...creatureData, visual: bestiaryEntry?.visual ?? creatureData.visual, footprint: bestiaryEntry?.footprint ?? creatureData.footprint }
        : creatureData;
      // Auto-roll attributes and create playable character fighter
      newFighter = createPlayableCharacterFighter(creatureWithVisual, nameToUse);
    ...
    // Force-hydrate visual/footprint from bestiary if missing (prevents modelUrl loss in later mapping layers)
    {
      const allCreatures = getAllBestiaryEntries(bestiary);
      const stripUnitSuffix = (s) =>
        String(s || "")
          .replace(/\s+#\d+$/i, "")
          .trim()
          .toLowerCase();
      const fighterBaseName = stripUnitSuffix(newFighter?.name);
      const sourceBaseName = stripUnitSuffix(creatureData?.name);
      const bestiaryMatch =
        allCreatures.find((c) => c.id === creatureData?.id) ||
        allCreatures.find((c) => stripUnitSuffix(c.name) === sourceBaseName) ||
        allCreatures.find((c) => stripUnitSuffix(c.name) === fighterBaseName);

      if (bestiaryMatch) {
        newFighter.bestiaryId = newFighter.bestiaryId || bestiaryMatch.id;
        if (bestiaryMatch.visual) {
          newFighter.visual = {
            ...(newFighter.visual || {}),
            ...bestiaryMatch.visual,
            modelUrl:
              newFighter?.visual?.modelUrl ??
              bestiaryMatch?.visual?.modelUrl ??
              null,
            desiredHeightFt:
              newFighter?.visual?.desiredHeightFt ??
              newFighter?.visual?.baseHeightFt ??
              bestiaryMatch?.visual?.desiredHeightFt ??
              6,
            baseHeightFt:
              newFighter?.visual?.baseHeightFt ??
              newFighter?.visual?.desiredHeightFt ??
              ...
              6,
            yawOffsetDeg:
              newFighter?.visual?.yawOffsetDeg ??
              bestiaryMatch?.visual?.yawOffsetDeg ??
              0,
          };
        }
        if (bestiaryMatch.footprint) {
          newFighter.footprint = {
            ...(newFighter.footprint || {}),
            ...bestiaryMatch.footprint,
            feet:
              newFighter?.footprint?.feet ??
              bestiaryMatch?.footprint?.feet ??
              5,
            radiusHex:
              newFighter?.footprint?.radiusHex ??
              bestiaryMatch?.footprint?.radiusHex ??
              0,
          };
        }
      }
    ...
    if (!isNoneish(magicAbilitiesStr) && typeof magicAbilitiesStr === "string") {
      ...
      if (spellResult.spells.length > 0) {
        ...
        // For Wizard magic: use level-based progression for player-side fighters,
        // keep unrestricted full catalog behavior for enemies/NPCs.
        if (isWizardMagic || spellResult.unrestricted) {
          if (newFighter.type === "player") {
            const wizardLevel = Number(level) || Number(newFighter.level) || 1;
            const bounded = buildWizardSpellbookForLevel({
              allSpells,
              level: wizardLevel,
            });
            newFighter.spellbook = bounded.spellbook;
            newFighter.magic = bounded.spellbook;
            newFighter.unrestricted = false;
            newFighter.magicProfile = { isWizardMagic: true, unrestricted: false };
          } else {
    ...
    // Normalize PPE using universal caster rules (PCs, NPC casters, monsters).
    {
      const beforePPE = Number(newFighter.PPE ?? 0) || 0;
      const normalizedPPE = normalizePPEState(newFighter, {
        rollMissingLevelGains: true,
        rng: createDeterministicRng(
          `${newFighter.id || newFighter.name || "fighter"}|spawn|${newFighter.level || 1}`
        ),
      });
      newFighter = { ...newFighter, ...normalizedPPE };
      if (newFighter.PPE > 0 && beforePPE !== newFighter.PPE) {
        addLog(
          `🔮 ${newFighter.name} PPE normalized (${newFighter.ppeType}): ${newFighter.currentPPE}/${newFighter.PPE}`,
          "info"
        );
      }
    }
    ...
    if (equippedWeapon) {
      ...
      if (isRanged && import.meta.env?.DEV && settingsRef.current?.showCombatDebug) {
        addLog(`Debug: ${newFighter.name} has ranged weapon ${equippedWeapon.name}`, "info");
      }
  ...
  function loadSavedPresetById(presetId) {
    ...
    const migratedFighters = migratePPEStateForCollection(data.fighters || []);
    resetAITransientRefs();
    setCombatActive(false);
    setFighters(migratedFighters);
  ...
  function handleDeleteSavedPreset(presetId) {
    const preset = savedPresets.find((entry) => entry.id === presetId);
    if (!preset) {
      addLog("❌ Saved preset not found", "error");
      return;
    }

    const confirmed = window.confirm(
      `Delete quick start preset "${preset.name}"?`
    );
    if (!confirmed) return;

    deleteSavedPreset(presetId);
    setSavedPresets(getSavedPresets());
    addLog(`🗑️ Deleted saved preset: ${preset.name}`, "success");
  }

  function startCombat(skipPhase0 = false) {
    let phase0Resolution = null;
    ...
    if (!skipPhase0 && combatTerrain && combatTerrain.mapType) {
      ...
      if (playerFighters.length > 0 && enemyFighters.length > 0) {
        try {
          phase0Resolution = resolvePhase0Encounter(
            playerFighters,
            enemyFighters,
    ...
    // Roll initiative for all fighters (1994 Palladium rules: d20 + bonuses)
      const hiddenPlayerIds = new Set(
        (phase0Resolution?.players || [])
          .filter((playerResult) => playerResult.hidden)
          .map((playerResult) => playerResult.player?.id)
          .filter(Boolean)
      );
      ...
      if (fighter.type === "player" && hiddenPlayerIds.has(fighter.id)) {
        updatedFighter.hidden = true;
        updatedFighter.isProwling = true;
        updatedFighter.prowlState = {
          ...(fighter.prowlState || {}),
          hidden: true,
          prowlSuccess: true,
          source: "phase0",
  ...
  async function executePsionicPower(userFighter, target, power) {
    ...
    revealFighterConcealment(userFighter, "psionic");
  ...
  async function executeSpell(caster, target, spell) {
    revealFighterConcealment(caster, "spell");
    const hasEngineCall = !!window?.engine?.call;
    const spellCasterId = caster?.id ?? caster ?? null;
    const allSpellsDbRef = getAllSpellsFromDB();
    const makeSpellResult = (ok, consumeAction, reason, extra = {}) => ({
      ok: !!ok,
      consumeAction: !!consumeAction,
      reason: reason || (ok ? "ok" : "failed"),
      ...extra,
    });
    const resolveSpellPpeCost = (spellLike) => {
      const direct = Math.max(0, Number(getSpellCost(spellLike) || 0));
      if (direct > 0) return direct;
      const spellName = String(spellLike?.name || "").toLowerCase().trim();
      if (!spellName) return 0;
      const dbMatch = (allSpellsDbRef || []).find(
        (sp) => String(sp?.name || "").toLowerCase().trim() === spellName
      );
      return Math.max(0, Number(getSpellCost(dbMatch) || 0));
    };
    const buildCastSnapshot = () => {
      const liveFighters = fightersRef.current || fighters;
      const livePositions =
        positionsRef.current && Object.keys(positionsRef.current).length > 0
          ? positionsRef.current
          : positions;
      const casterId = caster?.id ?? caster ?? null;
      const rawTargetId = target?.id ?? target ?? null;
      const rangeFt = getSpellRangeInFeet(spell);
      const resolvedTarget =
        (rawTargetId ? liveFighters.find((f) => f.id === rawTargetId) : null) ||
        (target?.id ? liveFighters.find((f) => f.id === target.id) : null) ||
        null;
      let targetId = resolvedTarget?.id ?? rawTargetId ?? null;
      const originPos = casterId ? livePositions?.[casterId] ?? null : null;
      // Normalize self-only spells (range 0) to self target at execution time.
      const normalizedSelfTarget = rangeFt === 0 && !!casterId;
      if (normalizedSelfTarget) {
        targetId = casterId;
      }
      const targetPos = targetId ? livePositions?.[targetId] ?? null : null;
      const distanceFt =
        originPos && targetPos ? calculateDistance(originPos, targetPos) : null;
      return {
        casterId,
        targetId,
        originPos,
        ...
        distanceFt,
        liveFighters,
        normalizedSelfTarget,
      };
    };
    const validateSnapshotRange = (snapshot) => {
      if (!snapshot?.targetId) {
        return {
          ok: false,
          reason: "no-target",
          log: `ℹ️ ${caster?.name || snapshot?.casterId} has no target for ${spell?.name || "spell"}.`,
        };
      }
      if (!snapshot.originPos || !snapshot.targetPos) {
        return {
          ok: false,
          reason: "target-position-unresolved",
          log: `❌ ${caster?.name || snapshot.casterId} cannot cast ${spell?.name || "spell"}: target position unresolved.`,
        };
      }
      const distanceIsFinite =
        typeof snapshot.distanceFt === "number" &&
        Number.isFinite(snapshot.distanceFt);
      const positionsDiffer =
        snapshot.originPos?.x !== snapshot.targetPos?.x ||
        snapshot.originPos?.y !== snapshot.targetPos?.y ||
        (snapshot.originPos?.z ?? snapshot.originPos?.altitudeFeet ?? 0) !==
          (snapshot.targetPos?.z ?? snapshot.targetPos?.altitudeFeet ?? 0);
      if (!distanceIsFinite || (positionsDiffer && snapshot.distanceFt <= 0)) {
        return {
          ok: false,
          reason: "invalid-distance",
          log: `❌ ${caster?.name || snapshot.casterId} cannot cast ${spell?.name || "spell"}: invalid distance data.`,
        };
      }
      if (
        snapshot.rangeFt !== Infinity &&
        snapshot.distanceFt != null &&
        snapshot.distanceFt > snapshot.rangeFt
      ) {
        return {
          ok: false,
          reason: "out-of-range",
          log: `❌ ${caster?.name || snapshot.casterId} cannot cast ${spell?.name || "spell"}: out of range (${Math.round(snapshot.distanceFt)}ft > ${snapshot.rangeFt}ft).`,
        };
      }
      return { ok: true, reason: "in-range" };
    };
    ...
    if (combatOverRef.current) {
      return makeSpellResult(false, false, "combat-over");
    }

    if (spellCasterId && spellCastInProgressRef.current.has(spellCasterId)) {
      addLog?.("⚠️ Spell cast already in progress, ignoring duplicate.", "warning");
      return makeSpellResult(false, false, "duplicate-in-progress");
    }
    if (spellCasterId) spellCastInProgressRef.current.add(spellCasterId);

    try {
      let didSpendResources = false;
      let didApplyImpact = false;
      const finalizeSpellResult = (ok, consumeAction, reason, extra = {}) =>
        makeSpellResult(
          ok,
          consumeAction || didSpendResources || didApplyImpact,
          reason,
          extra
        );

      const castSnapshot = buildCastSnapshot();
      const rangeValidation = validateSnapshotRange(castSnapshot);
      if (!rangeValidation.ok) {
        if (rangeValidation.log) addLog?.(rangeValidation.log, "warning");
        return finalizeSpellResult(false, false, rangeValidation.reason, {
          targetId: castSnapshot.targetId,
          rangeFt: castSnapshot.rangeFt,
          distanceFt: castSnapshot.distanceFt,
        });
      }

      // Non-engine fallback: still spend resources so PPE/ISP are authoritative in web mode.
      if (!hasEngineCall) {
        const casterId = castSnapshot.casterId;
        const targetId = castSnapshot.targetId;
        const ppeCost = resolveSpellPpeCost(spell);
        // executeSpell() is for magic casting; do not gate spells on ISP.
        const ispCost = 0;
        const liveFighters = castSnapshot.liveFighters;
        const liveCaster = liveFighters.find((f) => f.id === casterId);
        const ppeCostInfo = getAdjustedSpellPPECost(liveCaster, ppeCost);
        const spendPPECost = ppeCostInfo.adjustedCost;
        const curPPE = liveCaster?.currentPPE ?? liveCaster?.PPE ?? 0;
        const curISP = liveCaster?.currentISP ?? liveCaster?.ISP ?? 0;
        const effectivePoolPPE = getEffectivePPE(liveCaster, {
          onLeyLine: ppeCostInfo.onLeyLine,
          onNexus: ppeCostInfo.onNexus,
        });

        if (curPPE < spendPPECost || curISP < ispCost) {
          addLog?.(
            `❌ ${caster?.name || casterId} lacks resources (PPE ${curPPE}/${spendPPECost}` +
              `${spendPPECost !== ppeCost ? `; base ${ppeCost}` : ""}` +
              `${ppeCostInfo.onNexus ? "; nexus" : ppeCostInfo.onLeyLine ? "; ley line" : ""}` +
              `, effective pool ${effectivePoolPPE}, ISP ${curISP}/${ispCost})`,
            "error"
          );
          return finalizeSpellResult(false, false, "insufficient-resources", {
            targetId,
            rangeFt: castSnapshot.rangeFt,
            distanceFt: castSnapshot.distanceFt,
          });
        ...
        if (spendPPECost > 0 || ispCost > 0) {
          didSpendResources = true;
          setFighters((prev) =>
            prev.map((f) => {
              if (f.id !== casterId) return f;
              const ppe = f.currentPPE ?? f.PPE ?? 0;
              const isp = f.currentISP ?? f.ISP ?? 0;
              const nextPPE = Math.max(0, ppe - spendPPECost);
              const nextISP = Math.max(0, isp - ispCost);
              return {
                ...f,
                ...(spendPPECost > 0 ? { currentPPE: nextPPE } : {}),
                ...(ispCost > 0 ? { currentISP: nextISP } : {}),
              };
            })
          );
        }
        ...
        if (spendPPECost > 0) {
          const costSuffix =
            spendPPECost !== ppeCost
              ? ` (base ${ppeCost}${ppeCostInfo.onNexus ? ", nexus x3" : ppeCostInfo.onLeyLine ? ", ley line x2" : ""})`
              : "";
          addLog?.(`✨ ${caster?.name || casterId} spends ${spendPPECost} PPE (no-engine fallback)${costSuffix}.`, "info");
        }
        if (ispCost > 0) addLog?.(`✨ ${caster?.name || casterId} spends ${ispCost} ISP (no-engine fallback).`, "info");

        // No engine available: apply a deterministic in-client spell impact fallback
        // so casts are not "empty actions" in web mode.
        const damageFormula =
          spell?.damage ??
          spell?.combatDamage ??
          spell?.Damage ??
        ...
        if (targetId && damageFormula) {
          if (combatOverRef.current) {
            return finalizeSpellResult(false, false, "combat-over", {
              targetId,
              rangeFt: castSnapshot.rangeFt,
              distanceFt: castSnapshot.distanceFt,
            });
          }
          const roll = CryptoSecureDice.parseAndRoll(String(damageFormula));
          const damageTotal = Math.max(0, Number(roll?.totalWithBonus ?? 0));
          ...
          if (damageTotal > 0) {
            ...
            setFighters((prev) =>
              prev.map((f) => {
                if (f.id !== targetId) return f;
                const hp = f.currentHP ?? f.hp ?? f.HP ?? 0;
                const nextHP = Math.max(MIN_COMBAT_HP, hp - damageTotal);
                const hpStatus = getHPStatus(nextHP);
                return {
                  ...f,
                  currentHP: nextHP,
                  hp: nextHP,
                  HP: nextHP,
                  status: hpStatus.status,
                };
              })
            );
            addLog?.(
              `✨ ${caster?.name || casterId}'s ${spell?.name || "spell"} deals ${damageTotal} damage (no-engine fallback).`,
              "combat"
            );
          }
        }

        addLog?.("⚠️ Spell effect engine unavailable; impact resolved with local fallback.", "warning");
        return finalizeSpellResult(true, true, "fallback-impact", {
          targetId,
          rangeFt: castSnapshot.rangeFt,
          distanceFt: castSnapshot.distanceFt,
        });
      ...
      const targetKey = castSnapshot.targetId ?? target?.id ?? target ?? "self";
      ...
      const castId = `${turnCounter}:${castSnapshot.casterId}:${spellKey}:${targetKey}:${actionKey}`;

      if (activeCastIdsRef.current.has(castId)) {
        return finalizeSpellResult(false, false, "duplicate-cast-id"); // Already processed this cast (e.g. lock recovery retry)
      }
      ...
      const res = await window.engine.call("castSpell", {
        caster: castSnapshot.casterId,
        target: castSnapshot.targetId,
        spell,
        meta: { castId, flightMs: 550 },
        state,
      });

      addLog?.(
        `🧪 castSpell [${castId}] spell=${spell?.name || "spell"} caster=${castSnapshot.casterId} target=${castSnapshot.targetId ?? "none"} range=${castSnapshot.rangeFt === Infinity ? "∞" : castSnapshot.rangeFt} distance=${castSnapshot.distanceFt == null ? "n/a" : Math.round(castSnapshot.distanceFt)}`,
        "info"
      );
      ...
      if (!res?.ok) {
        ...
        addLog?.(`❌ Spell rejected: ${res?.error?.message}`, "error");
        return finalizeSpellResult(false, false, "engine-rejected", {
          targetId: castSnapshot.targetId,
          rangeFt: castSnapshot.rangeFt,
          distanceFt: castSnapshot.distanceFt,
        });
      ...
      // Failsafe spend: only apply if engine did NOT emit PPE_SPENT / ISP_SPENT.
      const casterId = caster?.id ?? caster;
      const events = Array.isArray(res?.events) ? res.events : [];
      const resolvedTargetFromEvents = events.find(
        (ev) =>
          ev?.targetId != null ||
          ev?.target != null ||
          ev?.targetEid != null ||
      ...
      if (resolvedTargetFromEvents) {
        const resolvedTargetId =
          resolvedTargetFromEvents.targetId ??
          resolvedTargetFromEvents.target ??
          resolvedTargetFromEvents.targetEid ??
          resolvedTargetFromEvents.tid;
        addLog?.(`🧪 castSpell resolvedTarget=${resolvedTargetId ?? "unknown"}`, "info");
      }
      const hasPpeSpentEvent = events.some((ev) => ev?.type === "PPE_SPENT" && (ev?.eid ?? ev?.id) === casterId);

      const fallbackPpeCost = !hasPpeSpentEvent ? resolveSpellPpeCost(spell) : 0;
      // executeSpell() fallback should only spend PPE; ISP belongs to executePsionicPower().
      const fallbackIspCost = 0;

      if (fallbackPpeCost > 0 || fallbackIspCost > 0) {
        const liveCaster = (fightersRef.current || fighters).find((f) => f.id === casterId);
        const ppeCostInfo = getAdjustedSpellPPECost(liveCaster, fallbackPpeCost);
        const spendFallbackPPECost = ppeCostInfo.adjustedCost;
        const beforePPE = liveCaster?.currentPPE ?? liveCaster?.PPE ?? 0;
        const beforeISP = liveCaster?.currentISP ?? liveCaster?.ISP ?? 0;
        const effectivePoolPPE = getEffectivePPE(liveCaster, {
          onLeyLine: ppeCostInfo.onLeyLine,
          onNexus: ppeCostInfo.onNexus,
        });

        // Safety: never allow fallback spend to push resources negative.
        if (beforePPE < spendFallbackPPECost || beforeISP < fallbackIspCost) {
          activeCastIdsRef.current.delete(castId);
          addLog?.(
            `❌ Resource sync guard blocked cast (${caster?.name || casterId}) PPE ${beforePPE}/${spendFallbackPPECost}` +
              `${spendFallbackPPECost !== fallbackPpeCost ? `; base ${fallbackPpeCost}` : ""}` +
              `${ppeCostInfo.onNexus ? "; nexus" : ppeCostInfo.onLeyLine ? "; ley line" : ""}` +
              `, effective pool ${effectivePoolPPE}, ISP ${beforeISP}/${fallbackIspCost}`,
            "error"
          );
          return finalizeSpellResult(false, false, "resource-sync-guard", {
            targetId: castSnapshot.targetId,
            rangeFt: castSnapshot.rangeFt,
            distanceFt: castSnapshot.distanceFt,
          });
        ...
        setFighters((prev) =>
          prev.map((f) => {
            if (f.id !== casterId) return f;
            const curPPE = f.currentPPE ?? f.PPE ?? 0;
            const curISP = f.currentISP ?? f.ISP ?? 0;
            const nextPPE = Math.max(0, curPPE - spendFallbackPPECost);
            const nextISP = Math.max(0, curISP - fallbackIspCost);
            return {
              ...f,
              ...(spendFallbackPPECost > 0 ? { currentPPE: nextPPE } : {}),
              ...(fallbackIspCost > 0 ? { currentISP: nextISP } : {}),
            };
          })
        );

        if (spendFallbackPPECost > 0) {
          const costSuffix =
            spendFallbackPPECost !== fallbackPpeCost
              ? ` (base ${fallbackPpeCost}${ppeCostInfo.onNexus ? ", nexus x3" : ppeCostInfo.onLeyLine ? ", ley line x2" : ""})`
              : "";
          addLog?.(`✨ ${caster?.name || casterId} spends ${spendFallbackPPECost} PPE (fallback)${costSuffix}.`, "info");
        }
        if (fallbackIspCost > 0) {
          addLog?.(`✨ ${caster?.name || casterId} spends ${fallbackIspCost} ISP (fallback).`, "info");
        }
      }
      ...
      return finalizeSpellResult(true, true, "engine-cast", {
        targetId: castSnapshot.targetId,
        rangeFt: castSnapshot.rangeFt,
        distanceFt: castSnapshot.distanceFt,
      });
      ...
      if (spellCasterId) {
        spellCastInProgressRef.current.delete(spellCasterId);
      }
    }
  }
  async function executeSelectedAction() {
    ...
    switch (actionToExecute.name) {
      case "Strike":
        if (targetToExecute && weaponToExecute) {
          if (selectedMovementHex) {
            dispatchCombatAction("STRIKE_WITH_MOVEMENT", {
      ...
      case "Prowl": {
        ...
        setFighters(prev => prev.map(f =>
          f.id === currentFighter.id
            ? {
              ...f,
              hidden: Boolean(currentFighter.prowlState?.hidden),
              isProwling: Boolean(currentFighter.prowlState?.hidden),
              prowlState: currentFighter.prowlState
                ? { ...currentFighter.prowlState }
                : { hidden: false, prowlSuccess: false },
              remainingAttacks: Math.max(0, f.remainingAttacks - (hideCost === "all" ? f.remainingAttacks : hideCost)),
            }
            : f
        ));
        scheduleEndTurn(500);
      ...
      case "Clerical Abilities": {
        ...
        revealFighterConcealment(currentFighter, "spell");
      ...
      case "Spells": {
        ...
        // Execute the spell (await — executeSpell is async, returns Promise)
        const spellResult = await executeSpell(currentFighter, targetToExecute, selectedSpell);
        const spellSuccess = spellResult?.ok === true;
        if (spellSuccess) {
          setSpellsMode(false);
          setSelectedSpell(null);

          // ✅ IMPORTANT: clear action-execution lock, or AI will freeze
          executingActionRef.current = false;
          clearTimeout(lockTimeout);
      ...
      case "Lift & Carry": {
        ...
        if (result.success) {
          const revealedCarrier = stripFighterConcealment(result.carrier, "movement");
          const revealedCarried = stripFighterConcealment(result.carried, "movement");
          setFighters(prev => prev.map(f => {
            if (f.id === currentFighter.id) return revealedCarrier;
            if (f.id === targetToExecute.id) return revealedCarried;
            return f;
          }));
          // Sync positions
          ...
          addLog(result.message, "info");
          if (isFighterConcealed(result.carrier)) {
            addLog(`👁️ ${result.carrier.name} is revealed while lifting and carrying a target!`, "info");
          }
          if (isFighterConcealed(result.carried)) {
            addLog(`👁️ ${result.carried.name} is revealed after being hauled into the air!`, "info");
          }
        } else {
          addLog(`❌ ${result.reason}`, "error");
        }
      ...
      case "Drop Target": {
        ...
        if (result.success) {
          const revealedCarrier = stripFighterConcealment(result.fighter, "movement");
          const revealedTarget = stripFighterConcealment(
            result.target,
            result.fallDamage ? "damage" : "movement"
          );
          setFighters(prev => prev.map(f => {
            if (f.id === currentFighter.id) return revealedCarrier;
            if (f.id === carried.id) return revealedTarget;
            return f;
          }));
          addLog(result.message, "warning");
          if (isFighterConcealed(result.fighter)) {
            addLog(`👁️ ${result.fighter.name} is revealed after dropping a carried target!`, "info");
          }
          if (isFighterConcealed(result.target)) {
            addLog(`👁️ ${result.target.name} is revealed by the forced fall!`, "info");
          }
      ...
      setFighters(prev => prev.map(f => {
        ...
        if ((!fighterCopy.PPE || fighterCopy.PPE === 0) && !isNoneish(fighterCopy.magicAbilities)) {
          ...
          if (spellResult.spells.length > 0) {
            ...
            // keep unrestricted full catalog behavior for enemies/NPCs.
            if (isWizardMagic || spellResult.unrestricted) {
              if (fighterCopy.type === "player") {
                const wizardLevel = Number(fighterCopy.level) || 1;
                const bounded = buildWizardSpellbookForLevel({
                  allSpells,
                  level: wizardLevel,
                });
                fighterCopy.spellbook = bounded.spellbook;
                fighterCopy.magic = bounded.spellbook;
                fighterCopy.unrestricted = false;
                fighterCopy.magicProfile = { isWizardMagic: true, unrestricted: false };
              } else {
        ...
        const normalizedPPE = normalizePPEState(fighterCopy, {
          rollMissingLevelGains: true,
          rng: createDeterministicRng(
            `${fighterCopy.id || fighterCopy.name || "fighter"}|reset|${fighterCopy.level || 1}`
          ),
        });
        Object.assign(fighterCopy, normalizedPPE);
        const maxPPE = fighterCopy.maxPPE !== undefined ? fighterCopy.maxPPE : (fighterCopy.PPE ?? 0);
  ...
  const selectParty = useCallback((party) => {
    ...
    const characterFighters = party.map(char => {
      ...
      const normalizedPartyPPE = normalizePPEState(
        {
          ...char,
          type: "player",
          magic: magicSpells,
          spellbook: magicSpells,
          knownSpells: magicSpells,
          unified,
        },
        {
          rollMissingLevelGains: true,
          rng: createDeterministicRng(
            `${char._id || char.id || char.name || "party-char"}|party|${char.level || 1}`
          ),
        }
      );
      const PPEValue = normalizedPartyPPE.PPE;
      const currentPPEValue = normalizedPartyPPE.currentPPE;
      const visibilityProfile = getFighterVisibilityProfile(char);

      const fighter = {
        ...char,
        id: `player-${char._id}-${generateCryptoId()}`,
        sourceCharacterId: char._id || char.id,
        ...
        maxPPE: normalizedPartyPPE.maxPPE,
        ppeType: normalizedPartyPPE.ppeType,
        ppeBase: normalizedPartyPPE.ppeBase,
        ppeLevelGainsTotal: normalizedPartyPPE.ppeLevelGainsTotal,
        ppeLevelGainRolls: normalizedPartyPPE.ppeLevelGainRolls,
        hasInfravision: visibilityProfile.hasInfravision,
        hasNightvision: visibilityProfile.hasNightvision,
        nightvisionRange: visibilityProfile.nightvisionRange,
        prowlSkill: visibilityProfile.prowlSkill,
        detectAmbushSkill: visibilityProfile.detectAmbushSkill,
        hasSpecialSenses: visibilityProfile.hasSpecialSenses,
        prowlState: char.prowlState || { hidden: false, prowlSuccess: false },
        hidden: Boolean(char.hidden || char.prowlState?.hidden),
    ...
    setSelectedParty,
    normalizeFighterId,
    getRandomAlignmentForRace,
    getFighterVisibilityProfile,
  ]);
    ...
    if (combatActive && activeFighters.length > 0 && !combatEndCheckRef.current) {
      ...
      const checkCombatEnd = () => {
        ...
        if (consciousPlayers.length === 0) {
          ...
          addLog("💀 All players are defeated! Enemies win!", "defeat");
          setCombatActive(false);
          cancelPendingCombatWork("defeat-delayed-check");
        } else if (consciousEnemies.length === 0) {
          ...
          addLog("🎉 Victory! All enemies defeated!", "victory");
          setCombatActive(false);
          cancelPendingCombatWork("victory-delayed-check");
        }
      };
  ...
  }, [fighters, combatActive, turnIndex, alivePlayers, aliveEnemies, canFighterAct, addLog, activeFighters.length, cancelPendingCombatWork]); // Check on turn changes
  ...
  const updateVisibleCells = useCallback(() => {
    ...
    const observerEntries = playerFighters.map(f => {
      const pos = positions[f.id];
      const altitude = f.altitudeFeet || f.altitude || 0;
      return {
        fighter: f,
        profile: getFighterVisibilityProfile(f),
        pos: { ...pos, altitude },
      };
    }).filter(entry => entry.pos && entry.pos.x !== undefined && entry.pos.y !== undefined);
    ...
    requestAnimationFrame(() => {
      ...
      observerEntries.forEach(({ fighter, profile, pos }) => {
        const observerRange = Math.max(
          baseRange,
          getVisibilityRange(lighting, profile.hasInfravision, fighter),
          profile.nightvisionRange || 0
        );
        const observerVisible = calculateVisibleCells(pos, observerRange, {
          terrain: combatTerrain || { lighting, mapType: "hex" },
          lighting,
          hasInfravision: profile.hasInfravision,
          isProwling: profile.isProwling,
          terrainObstacles: terrainObstaclesList,
          mapType: combatTerrain?.mapType || "hex",
          observerAltitude: pos.altitude || 0,
          fighterPositions: allFighterPositions,
        });

        observerVisible.forEach((cell) => {
          visibleMap.set(`${cell.x}-${cell.y}`, cell);
        });
      });
      ...
      if ((import.meta.env?.DEV || import.meta.env?.MODE === 'development') && visible.length > 0) {
          console.log(`[Fog of War] Calculated ${visible.length} visible cells from ${observerEntries.length} observer(s)`, {
          observerPositions: observerEntries.map((entry) => entry.pos),
          baseRange,
          lighting,
  ...
  }, [fogEnabled, combatTerrain, fighters, positions, exploredCells, getFighterVisibilityProfile]);
  ...
  return (
    ...
    >
                        ...
                        <Flex justify="space-between" align="start">
                          <VStack align="start" spacing={2} flex="1" w="100%">
                            <HStack flexWrap="wrap" spacing={2}>
                              ...
                              {fighter.prowlState?.hidden && (
                                <Badge colorScheme="green" size="md">
                                  Hidden
                                </Badge>
                              )}
                              {!fighter.prowlState?.hidden && fighter.prowlState?.prowlSuccess === false && (
                              ...
                              {fighter.hasInfravision && (
                                <Badge colorScheme="teal" size="md">
                                  Nightvision {fighter.nightvisionRange || 90}ft
                                </Badge>
                              )}
                              {fighter.hasSpecialSenses && (
                                <Badge colorScheme="pink" size="md">
                                  Special Senses
                                </Badge>
                              )}
                              {(() => {
                            ...
                            <Text fontSize="sm" color="blue.700">
                              ...
                              {(fighter.currentISP !== undefined || fighter.ISP !== undefined) &&
                                ` | ISP: ${fighter.currentISP ?? fighter.ISP ?? 0}/${fighter.ISP ?? fighter.currentISP ?? 0}`}
                              {(fighter.currentPPE !== undefined || fighter.PPE !== undefined) &&
                                ` | PPE: ${fighter.currentPPE ?? fighter.PPE ?? 0}/${fighter.PPE ?? fighter.currentPPE ?? 0}`}
                            </Text>
                            <Text fontSize="xs" color="gray.600">
                              Prowl: {fighter.prowlSkill || 0}% | Detect Ambush: {fighter.detectAmbushSkill || 0}%
                              {fighter.prowlState?.hidden ? " | Concealed" : ""}
                            </Text>
                  ...
                  <Flex direction="column" gap={4} width="100%" height="100%">
                    ...
                    <Box width="100%" minHeight="600px">
                      <TacticalMap
                        ...
                        visibilityRange={fighters
                          .filter(f => f.type === "player")
                          .reduce((maxRange, fighter) => {
                            const profile = getFighterVisibilityProfile(fighter);
                            return Math.max(
                              maxRange,
                              getVisibilityRange(
                                combatTerrain?.lighting || "BRIGHT_DAYLIGHT",
                                profile.hasInfravision,
                                fighter
                              ),
                              profile.nightvisionRange || 0
                            );
                          }, combatTerrain?.visibilityRange || getVisibilityRange(combatTerrain?.lighting || "BRIGHT_DAYLIGHT", false, null))}
                        mapHeight={mapHeight}
                                ...
                                <VStack align="stretch" spacing={3} fontSize="sm">
                                  <Box>
                                    ...
                                    <Text fontSize="sm" opacity={0.8} mt={1}>
                                      Nearest threat: {Number.isFinite(dangerDistance) ? `${dangerDistance} hex` : "—"}
                                    </Text>
                                    <Text fontSize="sm" opacity={0.8}>
                                    ...
                                    <Box fontSize="sm" opacity={0.85} mt={2}>
                                      <Text>
                                        Hovered: {hoveredCell ? `${hoveredCell.x},${hoveredCell.y}` : "—"}
                                      </Text>
                                      <Text>
                                        Selected: {selectedHex ? `${selectedHex.x},${selectedHex.y}` : "—"}
                                      </Text>
                                    </Box>
                                    {import.meta.env?.DEV && (
                                      <Box mt={2} p={2} borderWidth="1px" borderRadius="md">
                                        ...
                                        <Box>
                                          <RunActionLogger
                                            ...
                                            onUpdate={(updatedAttacker) =>
                                              dispatchCombatAction("RUN_UPDATE", {
      ...
      <LootWindow
        ...
        onTakeItem={async (item, itemType) => {
          const persisted = await persistLootToActiveParty([
            {
              ...item,
              type: itemType === "inventory" ? item.category || item.type : itemType,
            },
          ]);
          if (!persisted) return;
          ...
          addLog(
            `💰 ${item.name} taken from ${selectedLootSource?.name || "enemy"} and added to party inventory`,
            "info"
          );
        ...
        onTakeAll={async (allItems) => {
          const persisted = await persistLootToActiveParty(allItems);
          if (!persisted) return;
