...
export function hasActiveGrappleState(actor = {}, opponent = null) {
  ...
  const targetOpponent =
    opponent?.grappleState?.opponent ||
    opponent?.grappleState?.opponentId ||
    opponent?.grappledWith ||
    null;

  if (opponent?.id) {
    const actorLinked = actorOpponent === opponent.id && Boolean(state) && state !== "neutral";
    const targetLinked = targetOpponent === actor?.id && Boolean(opponentState) && opponentState !== "neutral";
    return Boolean(actorLinked || targetLinked);
  }

  return Boolean(
    (actorOpponent && state && state !== "neutral") ||
    /grapple|clinch|ground|pinned|held|restrained/.test(state)
  );
}
