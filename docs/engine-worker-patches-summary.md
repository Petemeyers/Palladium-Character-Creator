...
- **CombatPage:** `startNewMeleeRound(nextRoundNumber)` holds the melee reset (guards, status, `remainingAttacks`, etc.); `onEndTurnFull` applies worker result and calls `startNewMeleeRound` when `meleeRoundComplete`; fatigue/other “end turn” paths call `onEndTurnFull(currentFighter?.id)`.
- **Logging:** `eventToLogLine` handles `ROUND_STARTED`, `MELEE_ROUND_ENDED`.

---

## Patches 3–4: Payload Shape and Fast-Forward

- **Patch 3 – fightersLite and advanceUntilHuman**
  ...
  - **advanceUntilHuman:** Loops `END_TURN_FULL` until `isHumanLite(next)` or `maxSteps`; returns `nextState`, `events`, `stoppedBecause`.
  - **engine.worker.cjs:** Handles `advanceUntilHuman`.

- **Patch 4 – advanceUntilPlayer**
  - **resolveTurn.cjs:** `isPlayerControlledLite(f, playerSides)`; `advanceUntilPlayer` runs until a player-controlled turn, tracks `roundsAdvanced`, returns `roundsAdvanced` and `stoppedBecause: "PLAYER_TURN"`.
  - **CombatPage:** `onAdvanceUntilPlayer` calls it and runs `startNewMeleeRound(r)` for each advanced round.
  - **Patches 5–6:** `shouldLogEvent` filters noisy `TURN_STARTED`/`TURN_ENDED` during fast-forward; optional “Fast Forward to Player” behavior.

---

## Patch 7: Worker AI Action Selection

- **`src/engine/aiSelectAction.cjs`:** Pure logic: `canActLite`, distance, side checks; returns `{ type, targetId, reason }` (strike/move/defend/hold).
...
## Patch 9: AI Turn Orchestration (aiTakeTurn)
...
- **`src/engine/resolveAttack.cjs`:** `rollDie`, `rollDiceFormula`; `resolveAttack(payload)` does d20 + toHit vs AR, damage roll, crit mult, optional `state.hpById` and `ammo`; returns `{ events, delta }` (e.g. ATTACK_ROLL, DAMAGE, CRIT, MISS, HP_CHANGED, AMMO_SPENT).
...
- **resolveAttack.cjs:** Reads `attack.remainingAttacks`, decrements by 1, emits `ATTACKS_CONSUMED` and (when next is 0) `TURN_SHOULD_END`; delta includes `remainingAttacksById`.
- **CombatPage:** `applyAttackDelta` applies `delta.remainingAttacksById` to fighters; `executeAIAttack` sends `remainingAttacks` and, when events include `TURN_SHOULD_END`, calls `onEndTurnFull(attackerId)`.
...
- **CombatPage:** `runAITurnFromWorker` calls only `aiExecuteStep` (no separate aiTakeTurn/aiSelectAction); applies `delta.fightersLite` (HP, remainingAttacks) and `delta.ammoDelta`; logs events; runs `executeAIMove` when `step.kind === "MOVE"`; calls `onEndTurnFull` when `TURN_SHOULD_END` is present. Ammo overrides merged via `ammoOverrides` state.

---

## Patch 13: Real Profiles (attackProfilesById)

- **CombatPage:** `pickEquippedWeapon(f)`, `buildAttackProfilesById(fightersArr)` using `getWeaponDamage`, `isTwoHandedWeapon`, and base strike/AR; builds `{ baseStrikeBonus, damageFormula, ammoType, rangedRange, preferredRange, baseAR }` per id; passes `attackProfilesById` in `aiExecuteStep` state.
- **aiExecuteStep.cjs:** Uses `attackProfilesById[actorId]` / `[targetId]` for `toHitBonus`, `targetAR`, `damageFormula`, `ammoType` when resolving ATTACK.

---

## Patch 14A: UI Override (attackOverrideByKey)

- **aiExecuteStep.cjs:** Reads `state.attackOverrideByKey`; in ATTACK branch, `overrideKey = \`${actorId}::${targetId}::${attackMode}\``; uses `override?.toHitBonus ?? profile`, same for `targetAR`, `damageFormula`, `ammoType`, `critOn`, `critMult`.
...
- **CombatPage:** `computeAttackOverride(attacker, defender, attackMode)` uses: `getCoverBonus` for defender AR, `getWeaponDamage`/`pickEquippedWeapon` for damage/ammo, `getCombatBonus(attacker, "strike", attackSnapshot)` + `tempModifiers` for toHit, `applyLightingEffects` for lighting penalty.
- **Override flow:** When building override for ATTACK, uses `computeAttackOverride` instead of ad-hoc bonus/AR math so worker gets the same numbers as the rest of CombatPage.
...
- **Effect:** No separate “intent only” worker call; one logical AI step, with an optional second call only when the worker explicitly asks for an override.
...
|-------|-----------------|
| **Worker** | Turn advancement (`resolveTurn`, `advanceUntilPlayer`); AI intent (`aiSelectAction`); full step orchestration and attack resolution (`aiExecuteStep` → `resolveAttack`); action use and turn-end signals. |
| **UI** | Builds lite snapshots (`fightersLite`, `positionsLite`, `attackProfilesById`); responds to `REQUEST_ATTACK_OVERRIDE` with `computeAttackOverride`; applies deltas (HP, remainingAttacks, ammo); runs movement/pathing; logs; calls `onEndTurnFull` when worker says to. |
